...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		echo '<h4 class="attached-posts-section">' . sprintf( esc_attr__('Available %s', 'cmb'), $post_type_labels ) . '</h4>';
		...
		if ( ! $query_users ) {
			...
			echo '<p><button type="button" class="button cmb2-attached-posts-search-button" data-search=\''. esc_attr($js_data) .'\'>'. esc_attr($findtxt) .' <span title="'. esc_attr( $findtxt ) .'" class="dashicons dashicons-search"></span></button></p>';
		}
